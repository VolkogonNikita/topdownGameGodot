...
func save_game() -> void:
	...
	# ✅ Убраны лишние пробелы в ключах
...
func load_game() -> void:
	...
	# ✅ Ждём, пока сцена загрузится и игрок появится в дереве
	while get_tree().get_first_node_in_group("player") == null:
		await get_tree().process_frame

	# Теперь ссылки гарантированно найдутся
	initialize_references()
	...
	if file:
		...
		if player:
			# ✅ Исправлена опечатка global_posi tion
